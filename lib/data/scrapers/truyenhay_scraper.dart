...
class TruyenHayScraper extends BaseScraper {
  @override
  String get domain => 'truyenhay.app';
  ...
  Future<Novel> getNovelDetail(String url) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Selector có thể cần điều chỉnh nếu web truyenhay.app thay đổi giao diện
        String title = document.querySelector('h3.title')?.text.trim() ??
            document.querySelector('h1')?.text.trim() ?? 'Không rõ tên';
        String author = document.querySelector('a[href*="/tac-gia/"]')?.text.trim() ??
            document.querySelector('a[itemprop="author"]')?.text.trim() ??
            document.querySelector('.info a')?.text.trim() ?? 'Không rõ tác giả';
        String coverUrl = document.querySelector('.w-48 img')?.attributes['src'] ??
            document.querySelector('.flex-shrink-0 img')?.attributes['src'] ??
            document.querySelector('img.object-cover')?.attributes['src'] ??
            document.querySelector('.book img')?.attributes['src'] ?? '';
        ...
        throw Exception('Lỗi mạng khi tải chi tiết truyện từ truyenhay.app');
      }
    } catch (e) {
